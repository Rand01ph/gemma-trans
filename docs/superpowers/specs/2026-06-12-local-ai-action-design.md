...
`task=builtin.courier load=4.1s process=1.2s`）。**首例真实短信提取满分**：兔喜快递柜
取件短信 → `取件: 老乡鸡对面楼梯口2号兔喜快递柜 · 码 635383 · 48小时`（地点/码/时限
三项全对，格式严格符合契约）。待办正确创建；提醒事项小组件刷新延迟属 WidgetKit 固有
机制（系统时间线调度 + 刷新预算），非本方案可控——如需即时感知，快捷指令末尾加
「显示通知」动作（待补进指引）。
