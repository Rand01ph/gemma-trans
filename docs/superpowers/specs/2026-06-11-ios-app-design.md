...
**v1.x backlog（2026-06-12 真机反馈）**：① 长文本自动分段翻译（已立即实施：TextChunker
按段落/句子切 ≤700 块逐段译，UI 显示分段进度）；② **Safari 文章翻译**——分享 URL/链接给
app → 抓取网页正文（readability 提取）→ 分段翻译全文，前台 GPU 流式展示；依赖①就位，
是「沉浸式阅读」方向的第一步。③ UI 用户验收「中规中矩可接受」，本版定稿。

**通用本地 LLM 节点（v2 主线方向，2026-06-12）**：已打通的「App Intent 后台 → 本地引擎」
通路本质是**给 iOS 快捷指令生态提供一个不联网、不要 key 的本地大模型处理动作**；翻译只是
固定 prompt 的特例。泛化为 `text + instruction` 参数即可做：短信→提取待办、文本摘要/分类/
抽取、串进快捷指令自动化（读取内容 → 本地 AI 处理 → 写提醒/备忘）。定位升维：从「本地翻译」
到「本地 LLM 快捷指令引擎」，差异化强（系统翻译无、云端工具要联网+key）。
- **GPU 可用性是我们选的**：后台（openAppWhenRun=false）GPU 被 revoke 只能 CPU、但不跳转
  （翻译用此）；前台（openAppWhenRun=true）GPU 可用但切到本 app（可接受跳转的分析任务用此）。
  短文本 CPU 已够快（实测短句 0.3s），多数任务无需 GPU。
- 输出可经 `IntentResult & ReturnsValue` 回传快捷指令后续动作，串成自动化流。
- 限制：iOS 隐私不允许快捷指令任意读取全部短信；文本只能从分享/「收到信息」自动化/手动粘贴喂入。
- v1 先把翻译这个最小闭环做完上架验证，本方向作 v2 主线另立 spec。
