...
- **Spike 实测结论（2026-06-10/11，含 GUI 验证）**：✅ sandbox 内引擎加载成功；✅ Metal/WebGPU GPU 推理正常（真实翻译通过）；✅ `network.server` 生效；✅ 自动调优在沙盒内工作；✅ GPU 缓存写入容器内 Caches。
  - **取词关键结论（2026-06-11 实测，纠正先前推测）**：沙盒 **拒绝 AX 读取选中文本**（`kAXSelectedTextAttribute` 返回空，日志确凿），但 **CGEvent 模拟 ⌘C 取词在授予辅助功能权限后完全可用**（与 MAS 上 Bob 等划词工具同机制，GUI 实测弹出译文）。因此 **MAS 版无需任何降级**：与直分版共用 `SelectionReader.read()` 同一条 AX→⌘C 兜底链，沙盒里 AX 必然落到 ⌘C 兜底。先前"sandbox 禁止 CGEvent 注入"的假设作废，`MAS_BUILD` 编译开关随之移除。
