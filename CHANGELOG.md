...
- 翻译卡片右下角恢复生成速率，统计绑定当前请求，避免读取其他请求的结果。
- 长文章按原文段落完整翻译；输入上限仅限制单段，输出或上下文达到上限时自动拆分重试。
- 状态行显示分段进度；停止或出错保留已完成段落，不把部分结果标记为完成。

### Quality

- 增加 macOS UI 元素契约、浅色/深色截图基线、元素标注、差异检查与原生交互测试。
- 覆盖真实 MLX/GGUF 长文、取消以及 macOS 服务到卡片复制的验收。
